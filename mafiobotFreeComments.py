...
async def comment(interaction, recipient :discord.Member, anon :bool=True, message :str="", attachment :discord.Attachment=None):
	global playerChannels
	...
	if str(interaction.user.id) in playerChannels:
		senderPlayerChannel=client.get_channel(playerChannels[str(interaction.user.id)])
	if str(recipient.id) in playerChannels:
		recipientPlayerChannel=client.get_channel(playerChannels[str(recipient.id)])

	if senderPlayerChannel==None:
		await interaction.response.send_message(f'You don\'t have a player channel! Get a gm to do /set_player_channel to assign you one!', ephemeral=True)
	elif recipientPlayerChannel==None:
		await interaction.response.send_message(f'Your target doesn\'t have a player channel! Get a gm to do /set_player_channel to assign them one!', ephemeral=True)
	else:
		...
		loggedEmbed=discord.Embed(
			title=f'**You commented {recipient.display_name}**',
			description=f'{message}',
		...
		if not attachment==None:
			if not (attachment.url.endswith('.mp4')):
				...
				outboundEmbed.add_field(name="Attachment:",value=attachment.url,inline=False)
				loggedEmbed.add_field(name="Attachment:",value=attachment.url,inline=False)
...
async def on_ready():
	...
	dayChannel = client.get_channel(state.dayChannelID)
	await logChannel.send(f'The bot is alive!')
